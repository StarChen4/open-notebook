...
# This ensures offline environments can use token counting without network access
RUN mkdir -p /app/data/tiktoken-cache && \
    TIKTOKEN_CACHE_DIR=/app/data/tiktoken-cache \
    /app/.venv/bin/python -c "import tiktoken; tiktoken.get_encoding('o200k_base'); print('Tiktoken cache prepared')" || \
    echo "Warning: tiktoken preload failed, token counting will use fallback estimation"
