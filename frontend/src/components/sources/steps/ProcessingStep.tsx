...
export function ProcessingStep({
  ...
  return (
    <div className="space-y-8">
      <FormSection
        title="转换（可选）"
        description="应用 AI 转换来分析和提取内容中的见解。"
      >
        <CheckboxList
              ...
              render={({ field }) => (
                <label className="flex items-start gap-3 cursor-pointer p-3 rounded-md hover:bg-muted">
                  ...
                  <div className="flex-1">
                    <span className="text-sm font-medium block">启用嵌入以供搜索</span>
                    <p className="text-xs text-muted-foreground mt-1">
                      允许在向量搜索和 AI 查询中找到此资源
                    </p>
                  </div>
                </label>
              )}
          ...
          {settings?.default_embedding_option === 'always' && (
            <div className="p-3 rounded-md bg-primary/10 border border-primary/30">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <span className="text-sm font-medium block text-primary">嵌入已自动启用</span>
                  <p className="text-xs text-primary mt-1">
                    您的设置已配置为始终嵌入内容以供向量搜索。您可以在<span className="font-medium">设置</span>中更改此选项。
                  </p>
                </div>
              </div>
            </div>
          ...
          {settings?.default_embedding_option === 'never' && (
            <div className="p-3 rounded-md bg-muted border border-border">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <span className="text-sm font-medium block text-foreground">嵌入已禁用</span>
                  <p className="text-xs text-muted-foreground mt-1">
                    您的设置已配置为跳过嵌入。此资源将无法使用向量搜索。您可以在<span className="font-medium">设置</span>中更改此选项。
                  </p>
                </div>
              </div>
            </div>
