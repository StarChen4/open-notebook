        ...
        <CardContent className="space-y-6">
          <div className="space-y-3">
            ...
            <Collapsible open={expandedSections.doc} onOpenChange={() => toggleSection('doc')}>
              ...
              <CollapsibleContent className="mt-2 text-sm text-muted-foreground space-y-2">
                <p>• <strong>Docling</strong> 稍慢但更准确，特别是文档包含表格和图片时。</p>
                <p>• <strong>简单</strong> 将提取文档中的任何内容而不进行格式化。对于简单文档可以，但在复杂文档中会损失质量。</p>
                <p>• <strong>自动（推荐）</strong> 将尝试通过 docling 处理，如果失败则默认使用简单模式。</p>
              </CollapsibleContent>
            </Collapsible>
          </div>
          ...
          <div className="space-y-3">
            ...
            <Collapsible open={expandedSections.url} onOpenChange={() => toggleSection('url')}>
              ...
              <CollapsibleContent className="mt-2 text-sm text-muted-foreground space-y-2">
                <p>• <strong>Firecrawl</strong> 是一个付费服务（有免费层），功能非常强大。</p>
                <p>• <strong>Jina</strong> 也是一个不错的选择，也有免费层。</p>
                <p>• <strong>简单</strong> 将使用基本的 HTTP 提取，会遗漏基于 JavaScript 的网站内容。</p>
                <p>• <strong>自动（推荐）</strong> 将尝试使用 firecrawl（如果存在 API 密钥）。然后，它将使用 Jina 直到达到限制（或者如果您设置了 API 密钥则继续使用 Jina）。当前面的选项都不可用时，它将退回到简单模式。</p>
              </CollapsibleContent>
            </Collapsible>
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-3">
            ...
            <Collapsible open={expandedSections.embedding} onOpenChange={() => toggleSection('embedding')}>
              ...
              <CollapsibleContent className="mt-2 text-sm text-muted-foreground space-y-2">
                <p>嵌入内容将使您和您的 AI 代理更容易找到它。如果您运行本地嵌入模型（例如 Ollama），您不必担心成本，只需嵌入所有内容即可。对于在线提供商，只有在处理大量内容（例如每天数百个文档）时才需要小心。</p>
                <p>• 如果您运行本地嵌入模型或内容量不大，请选择<strong>总是</strong></p>
                <p>• 如果您想每次都决定，请选择<strong>询问</strong></p>
                <p>• 如果您不关心向量搜索或没有嵌入提供商，请选择<strong>从不</strong></p>
                <p>作为参考，OpenAI 的 text-embedding-3-small 每 100 万个令牌的成本约为 0.02 美元——大约是维基百科地球页面的 30 倍。使用 Gemini API，Text Embedding 004 是免费的，速率限制为每分钟 1500 个请求。</p>
              </CollapsibleContent>
            </Collapsible>
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-3">
            ...
            <Collapsible open={expandedSections.files} onOpenChange={() => toggleSection('files')}>
              ...
              <CollapsibleContent className="mt-2 text-sm text-muted-foreground space-y-2">
                <p>文件上传和处理后，不再需要它们。大多数用户应该允许 Open Notebook 自动从上传文件夹中删除已上传的文件。仅在将 Notebook 用作这些文件的主要存储位置时选择<strong>否</strong>（但您根本不应该这样做）。此选项将很快被弃用，改为始终下载文件。</p>
                <p>• 选择<strong>是</strong>（推荐）以在处理后自动删除已上传的文件</p>
                <p>• 仅在需要将原始文件保留在上传文件夹中时选择<strong>否</strong></p>
              </CollapsibleContent>
            </Collapsible>
          </div>
        </CardContent>
